...
class Experiment_Result():
    ...
    def find_diff_between_all_logs_anova(self, ground_truth_log_tuples, number_of_traces_array, alpha): # TODO

        logs = []
        for log_ind in range(len(number_of_traces_array)):
        ...
        if math.isnan(res.pvalue): ## TODO: check HACK: try a simple correction!
            for log in logs:
                if sum(log) == 0:
                    log[0] = 1
            res = stats.f_oneway(*logs)
    ...
    def add_experiment_result(self, T2Ps, k, min_diff, biases, alpha, statistical_diffs, number_of_trace, full_log_size):
        ...
        for transition in all_transitions:
                    ...
                    ground_truth_anova_was_significant = self.find_diff_between_all_logs_anova( \
                        all_transitions[transition], experiments_per_log, alpha)
                    actual_anova_was_significant = actual_transition_diffs_found['anova_test_significant']
        ...
        a_acc, a_power, a_precision, a_recall, a_statistical_error = self.compute_percision_metric(a_fn, a_fp, a_tn, a_tp)
        new_pairwise_row.extend([tp, fp, tn, fn, precision, recall, acc, statistical_error, power])
        new_anova_row.extend([a_tp, a_fp, a_tn, a_fn, a_precision, a_recall, a_acc, a_statistical_error, a_power])
    ...
    def export_to_csv(self, path):
        ...
        anova = path.replace('.csv', '_anova.csv')
    ...
    def export_summary_to_csv(self, path):
        ...
        path_anova = path.replace('.csv', '_anova.csv')
