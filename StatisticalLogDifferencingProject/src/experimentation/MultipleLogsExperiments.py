...
def get_logs(experiment_type, out_folder, full_log_size= 1000, biases = [0, 0.1, 0.1]):
    ...
    if experiment_type == 0:
        ...
        mozilla4_traces = log_parser.get_traces_as_lists_of_event_labels(mozilla4_traces, mozilla4_traces)
        mozilla5_traces = log_parser.get_traces_of_browser("Mozilla/5.0")
        mozilla5_traces = log_parser.get_traces_as_lists_of_event_labels(mozilla5_traces, mozilla5_traces)
...
if __name__ == '__main__':
    ...
    ks = [2]
    min_diffs = [0.01] #, 0.05, 0.1, 0.2, 0.4] #[0.01, 0.05, 0.1, 0.2, 0.4]
        ...
        for (min_diff, alpha) in itertools.product(min_diffs, alphas):
                ...
                for sample in traces_to_sample:
                    for trial in range(M): ## repeat the experiment for m randomally selected logs
                        ...
                        log2 = sample_traces(logs[0], sample)
                        log3 = sample_traces(logs[0], sample)
                        log4 = sample_traces(logs[0], sample)
